...
PortIn.totals = function(client, query, callback){
  if(arguments.length ===2){
    callback = query;
    query = null;
  }
  ...
  if (query) {
    client.makeRequest("get", client.concatAccountPath(PORT_IN_PATH) + "/" + "totals", query, function(err, res){
      if(err){
        return callback(err);
      }
      return callback(null, res);
    ...
    client.makeRequest("get", client.concatAccountPath(PORT_IN_PATH) + "/" + "totals", function(err, res){
      if(err){
        return callback(err);
      }
      return callback(null, res);
